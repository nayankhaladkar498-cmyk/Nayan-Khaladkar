// ==============================================================================
// EventSetu - Vendor Business Portal, Profile Builder & Photo Upload Engine
// Complete implementation of Add Your Business / Vendor Profile + Photo Upload Component
// ==============================================================================
...
const VendorApp = {
    ...
    try {
      ...
      if (!vendor) {
        vendor = {
          ...
          pricing_start: 75000,
          experience_years: 8,
          availability_info: 'Available all 7 days • Bookings open for 2026 & 2027 seasons',
  ...
  async renderFullVendorPortal(container, vendor) {
    ...
    const totalCount = bookings.length || 2;
    const pendingCount = bookings.filter(b => b.booking_status === 'pending').length;
    const acceptedCount = bookings.filter(b => b.booking_status === 'accepted' || b.booking_status === 'confirmed').length || 2;
    const totalEarnings = bookings
      .filter(b => b.booking_status === 'accepted' || b.booking_status === 'completed' || b.booking_status === 'confirmed')
    ...
    let html = `
          ...
          <div style="display:flex; gap:12px; align-items:center;">
            ...
            <div>
              ...
              <div style="margin-top:4px;">
                <span class="badge-tag-category" style="position:static; display:inline-block; font-size:0.7rem; padding:2px 8px; background:${vendor.is_approved ? '#dcfce7' : '#fef3c7'}; color:${vendor.is_approved ? '#15803d' : '#b45309'}; font-weight:700;">
                  ${vendor.is_approved ? '✓ Verified & Live on EventSetu' : '⏳ Pending Admin KYC Review'}
                </span>
      ...
      <div style="display:grid; grid-template-columns:repeat(2, 1fr); gap:10px; margin-bottom:16px;">
        <div style="background:#fff; border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:14px; box-shadow:var(--shadow-xs);">
          ...
          <div style="font-size:0.72rem; color:var(--primary); margin-top:2px;">Marketplace requests</div>
        </div>
        ...
        <div style="background:#fff; border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:14px; box-shadow:var(--shadow-xs);">
          ...
          <div style="font-size:0.72rem; color:var(--success); margin-top:2px;">Advance paid</div>
        </div>
        ...
        <div style="background:#fff; border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:14px; box-shadow:var(--shadow-xs);">
          ...
          <div style="font-size:0.72rem; color:var(--text-muted); margin-top:2px;">Direct UPI Payouts</div>
        </div>
      </div>
      ...
      <div style="display:flex; background:var(--bg-subtle); padding:4px; border-radius:var(--radius-full); margin-bottom:16px; border:1px solid var(--border-color); overflow-x:auto;">
        <button class="vendor-nav-tab ${this.activeTab === 'bookings' ? 'active' : ''}" onclick="window.EventSetuVendor.setTab('bookings')" style="flex:1; white-space:nowrap; padding:8px 8px; border:none; background:${this.activeTab === 'bookings' ? '#fff' : 'transparent'}; color:${this.activeTab === 'bookings' ? 'var(--primary)' : 'var(--text-muted)'}; font-weight:700; font-size:0.78rem; border-radius:var(--radius-full); box-shadow:${this.activeTab === 'bookings' ? 'var(--shadow-xs)' : 'none'}; cursor:pointer;">
          📋 Bookings
        </button>
        <button class="vendor-nav-tab ${this.activeTab === 'profile' ? 'active' : ''}" onclick="window.EventSetuVendor.setTab('profile')" style="flex:1; white-space:nowrap; padding:8px 8px; border:none; background:${this.activeTab === 'profile' ? '#fff' : 'transparent'}; color:${this.activeTab === 'profile' ? 'var(--primary)' : 'var(--text-muted)'}; font-weight:700; font-size:0.78rem; border-radius:var(--radius-full); box-shadow:${this.activeTab === 'profile' ? 'var(--shadow-xs)' : 'none'}; cursor:pointer;">
          🏢 Profile Info
        </button>
        <button class="vendor-nav-tab ${this.activeTab === 'packages' ? 'active' : ''}" onclick="window.EventSetuVendor.setTab('packages')" style="flex:1; white-space:nowrap; padding:8px 8px; border:none; background:${this.activeTab === 'packages' ? '#fff' : 'transparent'}; color:${this.activeTab === 'packages' ? 'var(--primary)' : 'var(--text-muted)'}; font-weight:700; font-size:0.78rem; border-radius:var(--radius-full); box-shadow:${this.activeTab === 'packages' ? 'var(--shadow-xs)' : 'none'}; cursor:pointer;">
          📦 Packages
        </button>
        <button class="vendor-nav-tab ${this.activeTab === 'gallery' ? 'active' : ''}" onclick="window.EventSetuVendor.setTab('gallery')" style="flex:1; white-space:nowrap; padding:8px 8px; border:none; background:${this.activeTab === 'gallery' ? '#fff' : 'transparent'}; color:${this.activeTab === 'gallery' ? 'var(--primary)' : 'var(--text-muted)'}; font-weight:700; font-size:0.78rem; border-radius:var(--radius-full); box-shadow:${this.activeTab === 'gallery' ? 'var(--shadow-xs)' : 'none'}; cursor:pointer;">
          🖼️ Photos Upload
        </button>
      </div>
    ...
    if (this.activeTab === 'bookings') {
      ...
      html += this.renderGalleryTabHtml(vendor);
  ...
  renderBookingsTabHtml(bookings) {
    ...
    bookings.forEach(b => {
      ...
      const isAccepted = b.booking_status === 'accepted' || b.booking_status === 'confirmed';
        ...
        <div style="background:#fff; border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:16px; margin-bottom:12px; box-shadow:var(--shadow-xs);">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; border-top:1px dashed var(--border-color); padding-top:10px; font-size:0.8rem;">
            ...
            <div style="display:flex; gap:6px;">
              ...
              ` : `
                <button class="btn btn-secondary btn-sm" onclick="window.EventSetuApp.showToast('Event completed! Payout settled.', 'success')">✓ Complete</button>
              `}
            </div>
          </div>
        </div>
      ...
      out += `
        <div style="background:#fff; border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:14px; margin-bottom:10px; box-shadow:var(--shadow-xs); display:flex; justify-content:space-between; align-items:center;">
          <div>
            ...
            <div style="font-size:0.8rem; color:var(--text-muted); margin-top:2px;">${s.description || 'Verified Package'}</div>
            <div style="font-size:0.75rem; color:var(--primary); font-weight:700; margin-top:2px;">Type: ${s.price_type || 'fixed'}</div>
          </div>
          <div style="text-align:right;">
  ...
  renderGalleryTabHtml(vendor) {
    const gallery = vendor.vendor_gallery || [];
    let out = `
      <div style="background:#fff; border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:16px; box-shadow:var(--shadow-xs); margin-bottom:16px;">
        <h4 style="font-size:1rem; font-weight:800; color:var(--text-main); margin-bottom:4px;">Upload Portfolio & Event Photos</h4>
        <p style="font-size:0.82rem; color:var(--text-muted); margin-bottom:14px;">High quality photos attract 4x more customer bookings. Max 5MB per image.</p>
        
        <!-- Interactive Dropzone Component -->
        <div class="photo-upload-dropzone" id="vendor-photo-dropzone" onclick="document.getElementById('vendor-file-input').click()">
          <input type="file" id="vendor-file-input" accept="image/jpeg,image/png,image/webp" style="display:none;" onchange="window.EventSetuVendor.handleFileSelect(event)">
          <div style="font-size:2rem; margin-bottom:6px;">📸</div>
          <strong style="font-size:0.92rem; color:var(--primary);">Click or Tap to Upload Photos</strong>
          <div style="font-size:0.75rem; color:var(--text-muted); margin-top:4px;">Supports JPG, PNG, WebP (Direct Supabase Cloud Storage)</div>
          <div id="upload-status-indicator" style="display:none; margin-top:10px; font-size:0.82rem; font-weight:700; color:var(--primary);">
            ⏳ Uploading to Cloud Storage...
          </div>
        </div>

        <div style="display:flex; gap:8px; margin-top:10px;">
          <button class="btn btn-secondary btn-sm" style="flex:1;" onclick="document.getElementById('vendor-file-input').click()">📁 Choose File</button>
          <button class="btn btn-secondary btn-sm" style="flex:1;" onclick="window.EventSetuVendor.promptAddPhotoUrl()">🔗 Add by URL</button>
        </div>
      </div>

      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
        <h4 style="font-size:0.95rem; font-weight:800; color:var(--text-main);">Current Gallery (${gallery.length} Photos)</h4>
      </div>
    `;

    if (gallery.length === 0) {
      out += `<div style="background:#fff; padding:24px; text-align:center; border-radius:var(--radius-lg); border:1px dashed var(--border-color); color:var(--text-muted);">No photos uploaded yet. Use the upload box above to add photos.</div>`;
      return out;
    }

    out += `<div class="gallery-preview-grid">`;
    gallery.forEach(g => {
      out += `
        <div class="gallery-preview-item">
          <img src="${g.image_url}" alt="Event Portfolio">
          <button class="gallery-remove-btn" onclick="window.EventSetuVendor.deletePhoto('${g.id}')" title="Delete photo">✕</button>
        </div>
      `;
    });
    out += `</div>`;
  ...
  renderProfileTabHtml(vendor) {
    return `
      <div style="background:#fff; border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:16px; box-shadow:var(--shadow-xs);">
        <h4 style="font-size:1.05rem; font-weight:800; color:var(--text-main); margin-bottom:4px;">🏢 Add / Edit Vendor Business Profile</h4>
        <p style="font-size:0.8rem; color:var(--text-muted); margin-bottom:16px;">This profile is displayed publicly to event hosts across Maharashtra.</p>
        ...
        <form onsubmit="event.preventDefault(); window.EventSetuVendor.saveProfileFromForm();">
          <div class="form-group">
            ...
            <input type="text" id="vp-input-bname" class="form-control" value="${vendor.business_name || ''}" placeholder="e.g. Green Valley Lawn & Banquet" required>
          </div>

          <div class="form-group">
            <label class="form-label">Owner / Contact Person *</label>
            <input type="text" id="vp-input-oname" class="form-control" value="${vendor.owner_name || ''}" placeholder="e.g. Nayan Khaladkar" required>
          </div>
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
            <div class="form-group">
              ...
              <select id="vp-input-cat" class="form-control">
                ...
                <option value="Wedding Planner" ${vendor.category === 'Wedding Planner' ? 'selected' : ''}>💍 Wedding Planner</option>
              </select>
            </div>
            <div class="form-group">
              <label class="form-label">Contact Mobile *</label>
              <input type="tel" id="vp-input-mobile" class="form-control" value="${vendor.mobile || ''}" placeholder="e.g. 7249593243" required>
            </div>
          </div>
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
            <div class="form-group">
              ...
              <input type="number" id="vp-input-price" class="form-control" value="${vendor.pricing_start || 25000}" required>
            </div>
            <div class="form-group">
              <label class="form-label">Experience (Years) *</label>
              <input type="number" id="vp-input-exp" class="form-control" value="${vendor.experience_years || 5}" min="0" required>
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <input type="text" id="vp-input-address" class="form-control" value="${vendor.address || 'Near Phase 1 IT Park, Hinjewadi, Pune 411057'}" required>
          </div>

          <div class="form-group">
            <label class="form-label">Availability Information *</label>
            <input type="text" id="vp-input-avail" class="form-control" value="${vendor.availability_info || 'Available all 7 days • Bookings open for 2026-2027'}" required>
          </div>

          <div class="form-group">
            <label class="form-label">Profile Cover Photo URL *</label>
            <div style="display:flex; gap:8px;">
              <input type="url" id="vp-input-photo" class="form-control" value="${vendor.profile_photo || ''}" placeholder="https://..." required>
              <button type="button" class="btn btn-secondary btn-sm" onclick="window.EventSetuVendor.promptProfilePhotoUpload()">Upload</button>
            </div>
          </div>
          ...
          <div class="form-group">
            <label class="form-label">Business Description & Services</label>
            <textarea id="vp-input-desc" class="form-control" rows="3" placeholder="Describe your venue/service, packages, amenities and USP...">${vendor.description || ''}</textarea>
          </div>

          <button type="submit" class="btn btn-primary" style="width:100%; margin-top:8px; padding:12px;">💾 Save Vendor Business Profile</button>
        </form>
      </div>
    `;
  },
  ...
  async handleFileSelect(event) {
    ...
    try {
      const uploadResult = await window.EventSetuUpload.uploadPhoto(file, 'vendor-gallery', this.currentVendor ? this.currentVendor.id : 'v_default');
      if (uploadResult && uploadResult.url) {
        ...
        window.EventSetuApp.showToast('Photo uploaded and added to gallery!', 'success');
        this.setTab('gallery');
      }
    } catch (err) {
      window.EventSetuApp.showToast(err.message || 'Image upload failed.', 'error');
    } finally {
  ...
  promptProfilePhotoUpload() {
    ...
    input.onchange = async (e) => {
      ...
      if (!file) return;
      try {
        window.EventSetuApp.showToast('Uploading profile image...', 'info');
        const res = await window.EventSetuUpload.uploadPhoto(file, 'vendor-profiles', this.currentVendor ? this.currentVendor.id : 'v_default');
        if (res && res.url) {
          const photoInput = document.getElementById('vp-input-photo');
          if (photoInput) photoInput.value = res.url;
          window.EventSetuApp.showToast('Profile image uploaded!', 'success');
        }
      } catch (err) {
        window.EventSetuApp.showToast(err.message || 'Upload error', 'error');
      }
    };
    input.click();
  },
  ...
  deletePhoto(photoId) {
    ...
    window.EventSetuDB.saveVendorProfile(this.currentVendor);
    window.EventSetuApp.showToast('Photo removed from gallery.', 'info');
    this.setTab('gallery');
  },
  ...
  async saveProfileFromForm() {
    ...
    this.currentVendor.address = document.getElementById('vp-input-address').value.trim();
    this.currentVendor.pricing_start = parseFloat(document.getElementById('vp-input-price').value) || 25000;
    this.currentVendor.experience_years = parseInt(document.getElementById('vp-input-exp').value) || 5;
    this.currentVendor.availability_info = document.getElementById('vp-input-avail').value.trim();
    this.currentVendor.profile_photo = document.getElementById('vp-input-photo').value.trim();
    ...
    await window.EventSetuDB.saveVendorProfile(this.currentVendor);
    window.EventSetuApp.showToast('Vendor Business Profile saved and synchronized!', 'success');
    this.loadVendorDashboard();
  }
};
