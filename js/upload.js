...
const UploadManager = {
  ...
  // Read file as Base64 Data URL for instant preview / fallback
  ...
  async uploadPhoto(file, bucketName = 'vendor-gallery', vendorId = 'default') {
    ...
    if (window.EventSetuConfig.isLiveSupabase && window.EventSetuConfig.client) {
      try {
        ...
        console.warn('Storage bucket upload encountered issue, falling back to secure data URL:', err);
      }
    }

    // Fallback: Convert to Base64 data URL for local presentation
